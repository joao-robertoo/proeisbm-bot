"""
BOT PROEISBM v7.1 Elite — Marica
...
from config import LOGIN, SENHA, INTERVALO_SEGUNDOS, CONVENIO_ALVO, TELEGRAM_TOKEN, TELEGRAM_CHAT_ID
...
def painel_status():
    ...
    return Panel(table, title=f"[bold red]PROEISBM MONITOR — {CONVENIO_ALVO}[/]", border_style="red", box=box.HEAVY)
...
def confirmar_inscricao(driver) -> bool:
    ...
    if any(p in pagina for p in PALAVRAS_NEGADO):
        ...
        notificar_inscricao_confirmada.__func__ if False else None  # noqa
        console.print()
        console.print(Panel(
            Align.center(Text.from_markup(
